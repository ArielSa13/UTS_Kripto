...
function determinant($matrix) // Menghitung determinan dari matriks 2x2.
{
    return $matrix[0][0] * $matrix[1][1] - $matrix[0][1] * $matrix[1][0];
}

function matrix_multiply($matrix, $scalar) // Mengalikan matriks dengan nilai skalar.
{
...
function matrix_modulo($matrix, $mod) // Melakukan operasi modulo pada setiap elemen matriks.
{
    for ($i = 0; $i < count($matrix); $i++) {
        for ($j = 0; $j < count($matrix[0]); $j++) {
...
function matrix_inverse($matrix, $mod) //Menghitung invers dari matriks 2x2 menggunakan aritmatika modulo.
{
